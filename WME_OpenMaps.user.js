...
async function onWmeReady() {
  ...
  var translations = {
    en: {
      ...
      active_maps_title_hint: 'Active maps — seven quick taps here toggles developer mode',
      waze_accred_tag_not_accredited: 'Not accredited',
      waze_not_listed_i_agree: 'I agree (this session)',
      waze_not_listed_eye_locked: 'Locked: this catalog map is not on Waze’s Map Data Attribution list. Use developer mode and confirm under Source & Waze.',
      developer_mode_enabled_log: 'Developer mode ON — catalog maps not on Waze’s list can be added; confirm each under Source & Waze for this session.',
      developer_mode_disabled_log: 'Developer mode OFF.',
      exit_developer_mode: 'Exit developer mode',
      exit_developer_mode_confirm: 'Turn off developer mode?\n\nSession confirmations (I agree) will be cleared. Maps not on Waze’s Map Data Attribution list will lock again until developer mode is re-enabled.',
      dev_not_listed_add_blocked: 'This map is not on Waze’s Map Data Attribution list. Enable developer mode (seven quick taps on the Active maps title) to add it.',
    ...
    nl: {
      ...
      active_maps_title_hint: 'Actieve kaarten — tik hier snel zeven keer om de ontwikkelaarsmodus aan of uit te zetten',
      waze_accred_tag_not_accredited: 'Niet geaccrediteerd',
      waze_not_listed_i_agree: 'Ik ga akkoord (deze sessie)',
      waze_not_listed_eye_locked: 'Vergrendeld: deze cataloguskaart staat niet op de Waze-pagina Map Data Attribution. Schakel de ontwikkelaarsmodus in en bevestig onder Bron & Waze.',
      developer_mode_enabled_log: 'Ontwikkelaarsmodus AAN — cataloguskaarten die niet op de Waze-lijst staan, kunnen worden toegevoegd; bevestig elke kaart onder Bron & Waze voor deze sessie.',
      developer_mode_disabled_log: 'Ontwikkelaarsmodus UIT.',
      exit_developer_mode: 'Ontwikkelaarsmodus afsluiten',
      exit_developer_mode_confirm: 'Ontwikkelaarsmodus uitzetten?\n\nSessiebevestigingen (Ik ga akkoord) worden gewist. Kaarten die niet op Map Data Attribution staan, worden weer vergrendeld tot de ontwikkelaarsmodus weer aan staat.',
      dev_not_listed_add_blocked: 'Deze kaart staat niet op Map Data Attribution van Waze. Schakel de ontwikkelaarsmodus in (zeven snelle tikken op de titel Actieve kaarten) om toe te voegen.',
    ...
    fr: {
      ...
      active_maps_title_hint: 'Cartes actives — sept appuis rapides ici activent ou désactivent le mode développeur',
      waze_accred_tag_not_accredited: 'Non accrédité',
      waze_not_listed_i_agree: 'J’accepte (cette session)',
      waze_not_listed_eye_locked: 'Verrouillé : cette carte catalogue n’est pas sur la page Map Data Attribution de Waze. Activez le mode développeur et confirmez sous Source et Waze.',
      developer_mode_enabled_log: 'Mode développeur ACTIF — les cartes catalogue absentes de la liste Waze peuvent être ajoutées ; confirmez chaque carte sous Source et Waze pour cette session.',
      developer_mode_disabled_log: 'Mode développeur désactivé.',
      exit_developer_mode: 'Quitter le mode développeur',
      exit_developer_mode_confirm: 'Désactiver le mode développeur ?\n\nLes confirmations de session (J’accepte) seront effacées. Les cartes absentes de Map Data Attribution seront verrouillées jusqu’à réactivation du mode développeur.',
      dev_not_listed_add_blocked: 'Cette carte n’est pas sur Map Data Attribution de Waze. Activez le mode développeur (sept appuis rapides sur le titre Cartes actives) pour l’ajouter.',
    ...
    'pt-BR': {
      ...
      active_maps_title_hint: 'Mapas ativos — sete toques rápidos aqui alternam o modo de programador',
      waze_accred_tag_not_accredited: 'Não credenciado',
      waze_not_listed_i_agree: 'Concordo (esta sessão)',
      waze_not_listed_eye_locked: 'Bloqueado: este mapa de catálogo não está em Map Data Attribution da Waze. Ative o modo de programador e confirme em Fonte e Waze.',
      developer_mode_enabled_log: 'Modo de programador LIGADO — mapas de catálogo fora da lista Waze podem ser adicionados; confirme cada um em Fonte e Waze nesta sessão.',
      developer_mode_disabled_log: 'Modo de programador DESLIGADO.',
      exit_developer_mode: 'Sair do modo de programador',
      exit_developer_mode_confirm: 'Desligar o modo de programador?\n\nAs confirmações de sessão (Concordo) serão limpas. Mapas fora de Map Data Attribution voltam a ficar bloqueados até voltar a ligar o modo de programador.',
      dev_not_listed_add_blocked: 'Este mapa não está em Map Data Attribution da Waze. Ative o modo de programador (sete toques rápidos no título Mapas ativos) para o adicionar.',
...
// --- TERMS OF USE REGISTRY ---
  ...
  /** Catalog map whose Waze accreditation status is `not_listed` (user maps never qualify). */
  function openMapsIsCatalogWazeNotListedMap(map) {
    return !!(map && openMapsWazeAccreditationStatus(map).code === 'not_listed');
  }
  ...
  /** Shared compact chip look for map card + Source & Waze summary when status is `not_listed` (UA button reset for height parity with type chips). */
  function openMapsWazeNotListedAccredPillCss(boxBorder, bg, fg) {
    return 'display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;box-sizing:border-box;margin:0;min-height:0;max-height:none;height:auto;appearance:none;-webkit-appearance:none;-moz-appearance:none;font-size:9px;font-weight:600;padding:1px 5px;border-radius:4px;border:1px solid ' + boxBorder + ';background:' + bg + ';color:' + fg + ';line-height:1.15;font-family:inherit;white-space:nowrap;vertical-align:middle;';
  }

  var OPEN_MAPS_WAZE_NOT_LISTED_SESSION_STORAGE_KEY = 'openmaps-waze-not-listed-agreed';

  function openMapsWazeNotListedSessionIdKeys(id) {
    ...
    if (typeof n === 'number' && !isNaN(n) && isFinite(n)) {
      var ns = String(n);
      if (keys.indexOf(ns) === -1) keys.push(ns);
    }
    return keys;
  }

  function openMapsWazeNotListedSessionReadObj() {
    try {
      var raw = sessionStorage.getItem(OPEN_MAPS_WAZE_NOT_LISTED_SESSION_STORAGE_KEY);
      if (!raw) return {};
  ...
  function openMapsWazeNotListedSessionWriteObj(obj) {
    try {
      sessionStorage.setItem(OPEN_MAPS_WAZE_NOT_LISTED_SESSION_STORAGE_KEY, JSON.stringify(obj || {}));
    } catch (eWr) { /* ignore */ }
  }

  function openMapsWazeNotListedSessionHasAgreed(id) {
    var o = openMapsWazeNotListedSessionReadObj();
    var keys = openMapsWazeNotListedSessionIdKeys(id);
    for (var hi = 0; hi < keys.length; hi++) {
      if (o[keys[hi]] === true || o[keys[hi]] === 1) return true;
    }
    return false;
  }

  function openMapsWazeNotListedSessionSetAgreed(id) {
    if (id == null || id === '') return;
    var o = openMapsWazeNotListedSessionReadObj();
    var keys = openMapsWazeNotListedSessionIdKeys(id);
    ...
    openMapsWazeNotListedSessionWriteObj(o);
  }

  function openMapsWazeNotListedSessionClearAll() {
    try {
      sessionStorage.removeItem(OPEN_MAPS_WAZE_NOT_LISTED_SESSION_STORAGE_KEY);
    } catch (eClr) { /* ignore */ }
  }

  /**
   * Runtime gate: catalog `not_listed` maps are unusable until developer mode + per-session I agree.
   * @param {*} handleMapId Prefer MapHandle.mapId (string/number parity vs catalog map.id).
   */
  function openMapsWazeNotListedLayerUseBlocked(map, handleMapId) {
    if (!openMapsIsCatalogWazeNotListedMap(map)) return false;
  ...
  var Settings = {
    'get': function() {
      ...
      if (typeof settings.developerMode === 'undefined') {
  ...
  function openMapsRefreshAllWazeNotListedGates() {
    handles.forEach(function(h) {
      if (h && typeof h.applyWazeNotListedGate === 'function') {
        try {
          h.applyWazeNotListedGate();
        } catch (eG) { /* ignore */ }
      }
    });
    try {
...
// --- CACHED SATELLITE IMAGERY TOGGLE ---
  ...
  var exitDevBtn = null;
  var syncExitDevFooterVisibility = function() {};
  ...
  title.title = I18n.t('openmaps.active_maps_title_hint');
  var openMapsDevModeTapCount = 0;
  var openMapsDevModeTapTimer = null;
  var OPEN_MAPS_DEV_MODE_TAP_WINDOW_MS = 2800;
  title.addEventListener('click', function(ev) {
    ev.preventDefault();
    ev.stopPropagation();
    openMapsDevModeTapCount++;
    if (openMapsDevModeTapTimer) clearTimeout(openMapsDevModeTapTimer);
    openMapsDevModeTapTimer = setTimeout(function() {
      openMapsDevModeTapCount = 0;
      openMapsDevModeTapTimer = null;
    }, OPEN_MAPS_DEV_MODE_TAP_WINDOW_MS);
    if (openMapsDevModeTapCount < 7) return;
    openMapsDevModeTapCount = 0;
    if (openMapsDevModeTapTimer) {
      clearTimeout(openMapsDevModeTapTimer);
      openMapsDevModeTapTimer = null;
    }
    var cur = !!Settings.get().developerMode;
    Settings.set('developerMode', !cur);
    if (cur) {
      openMapsWazeNotListedSessionClearAll();
    }
    openMapsInvalidateAddListCaches();
    if (typeof addMapSuggestions !== 'undefined' && addMapSuggestions && addMapSuggestions.style.display === 'block') {
      try {
    ...
    try {
      syncExitDevFooterVisibility();
    } catch (eF) { /* ignore */ }
    try {
      log(!cur ? I18n.t('openmaps.developer_mode_enabled_log') : I18n.t('openmaps.developer_mode_disabled_log'));
    } catch (eL) { /* ignore */ }
  });
...
// --- NATIVE SEARCHABLE MAP SELECTOR ---
  ...
  exitDevBtn = document.createElement('button');
  exitDevBtn.type = 'button';
  exitDevBtn.className = 'openmaps-sidebar-footer-exit-dev openmaps-wz-btn-compact';
  exitDevBtn.textContent = I18n.t('openmaps.exit_developer_mode');
  exitDevBtn.style.cssText = 'display:none;flex:0 0 auto;padding:4px 10px;height:32px;font-size:12px;border:1px solid #dadce0;border-radius:6px;background:#fff;color:#1a73e8;cursor:pointer;font-family:inherit;';
  exitDevBtn.addEventListener('click', function(ev) {
    ev.preventDefault();
    ev.stopPropagation();
    if (!Settings.get().developerMode) return;
    if (!confirm(I18n.t('openmaps.exit_developer_mode_confirm'))) return;
    ...
    syncExitDevFooterVisibility();
    try {
      log(I18n.t('openmaps.developer_mode_disabled_log'));
    } catch (eL2) { /* ignore */ }
  });
  syncExitDevFooterVisibility = function() {
    if (exitDevBtn) exitDevBtn.style.display = Settings.get().developerMode ? 'inline-flex' : 'none';
  };
  syncExitDevFooterVisibility();
  footer.appendChild(footerVersion);
  footer.appendChild(exitDevBtn);
...
function getNotAddedMaps() {
    var out = [];
    var devOn = !!Settings.get().developerMode;
    maps.forEach(function(map) {
      if (map.type === 'GOOGLE_MY_MAPS' && !openMapsGoogleMyMapsIntegrationEnabled()) return;
      if (!devOn && openMapsIsCatalogWazeNotListedMap(map)) return;
  ...
  function applyActiveMapsFilter() {
    ...
    handles.forEach(function(h) {
      ...
      if (show && !Settings.get().developerMode && openMapsIsCatalogWazeNotListedMap(map)) show = false;
...
function selectMapToAdd(mapId) {
    ...
    if (!addedMap && mapId != null && mapId !== '') {
      var n0 = Number(mapId);
      if (!isNaN(n0)) addedMap = maps.get(n0);
    }
    if (!addedMap) return;
    ...
    if (!Settings.get().developerMode && openMapsIsCatalogWazeNotListedMap(addedMap)) {
      try {
        log(I18n.t('openmaps.dev_not_listed_add_blocked'));
      } catch (eLog) {
        console.info('[OpenMaps] ' + (addedMap.title || mapId));
      }
      return;
    }
...
function MapHandle(map, options) {
    ...
    var UI = { touDetails: null, syncSourceWazeSection: null, mapLayersNoActiveMark: null, mapLayersSubContainer: null, mapLayersDetailsRoot: null, orphanHintEl: null, layerCatalogLoadingEl: null, accredPillRow: null };
...
var handle = document.createElement('div');
      ...
      titleRow.style.cssText = 'display:flex; align-items:center; flex-wrap:wrap; gap:6px; min-width:0; width:100%;';

      UI.title = document.createElement('div');
      UI.title.className = 'open-maps-title';
      UI.title.style.cssText = 'cursor:pointer; font-weight:normal; font-size:13px; color:var(--content_primary, #3c4043); white-space:normal; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; line-height:1.2; flex:1; min-width:0;';
      UI.title.textContent = map.title;
      UI.title.addEventListener('click', function(e) {
        e.stopPropagation();
        if (openMapsWazeNotListedLayerUseBlocked(map, self.mapId)) {
          if (UI.touDetails) {
            UI.editContainer.style.display = 'block';
            if (UI.editBtn) UI.editBtn.style.transform = 'rotate(180deg)';
            UI.touDetails.open = true;
            try {
              UI.touDetails.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            } catch (eSc) { /* ignore */ }
          }
          return;
        }
        UI.visibility.dispatchEvent(new MouseEvent('click'));
      });

      var nlPillBs = openMapsAccredBadgeStyle('not_listed');
      UI.notAccreditedPill = document.createElement('button');
      UI.notAccreditedPill.type = 'button';
      UI.notAccreditedPill.className = 'openmaps-not-accredited-pill';
      UI.notAccreditedPill.style.cssText = 'display:none;' + openMapsWazeNotListedAccredPillCss(nlPillBs.boxBorder, nlPillBs.bg, nlPillBs.fg) + 'cursor:pointer;';
      UI.notAccreditedPill.textContent = I18n.t('openmaps.waze_accred_tag_not_accredited');
      UI.notAccreditedPill.setAttribute('aria-label', I18n.t('openmaps.waze_accred_tag_not_accredited'));
      UI.notAccreditedPill.addEventListener('click', function(ev) {
        ev.preventDefault();
        ev.stopPropagation();
        if (UI.touDetails) {
          UI.editContainer.style.display = 'block';
      ...
      var accredPillRow = document.createElement('div');
      accredPillRow.className = 'openmaps-map-card-accred-pill-row';
      accredPillRow.style.cssText = 'display:none;align-items:center;width:100%;min-width:0;margin-top:2px;';
      accredPillRow.appendChild(UI.notAccreditedPill);
      textContainer.appendChild(accredPillRow);
      UI.accredPillRow = accredPillRow;
...
// --- BLEND MODE DROPDOWN ---
      ...
      function appendMapLayersSummaryInner(summaryEl) {
        ...
        UI.mapLayersActionsBtn = actionsBtn;
        ...
        showAllBtn.addEventListener('click', function(e) {
          ...
          e.stopPropagation();
          closeMenu();
          if (openMapsWazeNotListedLayerUseBlocked(map, self.mapId)) return;
      ...
      function appendOneLayerRow(layerItem, container) {
        ...
        self.layerUI.push({
          item: layerItem, badge: lBadge, title: lTitle, desc: lDesc, queryBtn: lQuery, eyeBtn: lVis,
      ...
      function applyCatalogFromEntry(entry) {
        ...
        saveMapState();
        try {
          if (typeof self.applyWazeNotListedGate === 'function') self.applyWazeNotListedGate();
        } catch (eCatGate) { /* ignore */ }
      }
      ...
      if (map.type === 'WMS' || map.type === 'ESRI') {
        ...
        function openMapsAccredBadgeOpenDetails() {
          if (UI.touDetails) {
            UI.editContainer.style.display = 'block';
            if (UI.editBtn) UI.editBtn.style.transform = 'rotate(180deg)';
            UI.touDetails.open = true;
        ...
        accBadge.addEventListener('click', function(ev) {
          ev.preventDefault();
          ev.stopPropagation();
          if (openMapsWazeAccreditationStatus(map).code !== 'not_listed') return;
          if (!Settings.get().developerMode) return;
          var ag = openMapsWazeNotListedSessionHasAgreed(self.mapId) || openMapsWazeNotListedSessionHasAgreed(map.id);
          if (ag) return;
          openMapsAccredBadgeOpenDetails();
        });
        accBadge.addEventListener('keydown', function(ev) {
          if (ev.key !== 'Enter' && ev.key !== ' ') return;
          ev.preventDefault();
          ev.stopPropagation();
          if (openMapsWazeAccreditationStatus(map).code !== 'not_listed') return;
          if (!Settings.get().developerMode) return;
          var ag2 = openMapsWazeNotListedSessionHasAgreed(self.mapId) || openMapsWazeNotListedSessionHasAgreed(map.id);
          if (ag2) return;
          openMapsAccredBadgeOpenDetails();
        });
        ...
        var wazeAgreeBtn = document.createElement('button');
        wazeAgreeBtn.type = 'button';
        wazeAgreeBtn.className = 'openmaps-waze-not-listed-agree-btn';
        wazeAgreeBtn.style.cssText = 'display:none;margin-bottom:10px;padding:6px 12px;font-size:12px;border-radius:6px;border:1px solid #dadce0;background:#fff;color:#202124;cursor:pointer;font-family:inherit;font-weight:600;';
        wazeAgreeBtn.textContent = I18n.t('openmaps.waze_not_listed_i_agree');
        wazeAgreeBtn.addEventListener('click', function(ev) {
          ev.preventDefault();
          ev.stopPropagation();
          openMapsWazeNotListedSessionSetAgreed(self.mapId);
          if (map.id != null && map.id !== '') openMapsWazeNotListedSessionSetAgreed(map.id);
          if (typeof self.applyWazeNotListedGate === 'function') {
            self.applyWazeNotListedGate();
            setTimeout(function() {
              try {
                self.applyWazeNotListedGate();
              } catch (eAg1) { /* ignore */ }
        ...
        wazeBox.appendChild(wazeAgreeBtn);
        ...
        function syncSourceWazeSection() {
          ...
          var devOn = !!Settings.get().developerMode;
          var agreed = openMapsWazeNotListedSessionHasAgreed(self.mapId) || openMapsWazeNotListedSessionHasAgreed(map.id);
          if (st.code === 'not_listed') {
            if (agreed) {
              accBadge.style.cssText = 'display:none;';
              accBadge.textContent = '';
              accBadge.removeAttribute('role');
              accBadge.removeAttribute('tabindex');
              accBadge.removeAttribute('aria-label');
            } else {
              accBadge.style.cssText = openMapsWazeNotListedAccredPillCss(bs.boxBorder, bs.bg, bs.fg) + 'margin-left:4px;';
              var tagTxt = I18n.t('openmaps.waze_accred_tag_not_accredited');
              accBadge.textContent = tagTxt;
              accBadge.setAttribute('aria-label', tagTxt);
              if (devOn) {
                accBadge.setAttribute('role', 'button');
                accBadge.tabIndex = 0;
                accBadge.style.cursor = 'pointer';
              } else {
                accBadge.removeAttribute('role');
                accBadge.removeAttribute('tabindex');
                accBadge.style.cursor = 'default';
              }
            }
          } else {
            accBadge.style.cssText = 'font-size:10px; font-weight:600; padding:2px 6px; border-radius:10px; margin-left:4px; white-space:nowrap;';
          ...
          formA.href = openMapsBuildLayerRequestFormUrl(map);
          if (st.code === 'not_listed' && devOn && !agreed) {
            wazeAgreeBtn.style.display = 'block';
          } else {
            wazeAgreeBtn.style.display = 'none';
          }
        }
      ...
      self.__openmapsRebuildMapLayersUI = rebuildMapLayersUI;
    }

    this.applyWazeNotListedGate = function() {
      var blocked = openMapsWazeNotListedLayerUseBlocked(map, self.mapId);
      if (!openMapsIsCatalogWazeNotListedMap(map)) {
        self.__openmapsWazeNotListedGateBlocked = false;
        self.__openmapsWazeNotListedStash = null;
        var cbx0 = layerToggler.querySelector('wz-checkbox');
        if (cbx0) cbx0.disabled = false;
        try {
          if (typeof self.__openmapsRebuildMapLayersUI === 'function') self.__openmapsRebuildMapLayersUI();
        } catch (eRb0) { /* ignore */ }
        if (typeof self.updateVisibility === 'function') self.updateVisibility();
        return;
      }

      var wasBlocked = !!self.__openmapsWazeNotListedGateBlocked;

      if (blocked && !wasBlocked) {
        self.__openmapsWazeNotListedStash = {
          hidden: self.hidden,
          mapLayers: self.mapLayers.map(function(l) { return { name: l.name, visible: !!l.visible }; })
        };
        self.hidden = true;
        self.mapLayers.forEach(function(l) { l.visible = false; });
        self.__openmapsWazeNotListedGateBlocked = true;
        if (typeof self.updateLayers === 'function') self.updateLayers();
      } else if (!blocked && wasBlocked) {
        var st = self.__openmapsWazeNotListedStash;
        if (st) {
        ...
        if (ch && typeof self.updateLayers === 'function') self.updateLayers();
        self.__openmapsWazeNotListedGateBlocked = true;
      } else {
        self.__openmapsWazeNotListedGateBlocked = blocked;
      }

      var cbxM = layerToggler.querySelector('wz-checkbox');
      if (cbxM) {
        cbxM.disabled = !!blocked;
        if (blocked) {
          cbxM.checked = false;
...
// --- UNIFIED VISIBILITY CONTROLLER ---
    this.setManualVisibility = function(wantsVisible) {
      if (openMapsWazeNotListedLayerUseBlocked(map, self.mapId)) {
        if (wantsVisible) {
          var checkboxBlock = layerToggler.querySelector('wz-checkbox');
          if (checkboxBlock) {
            checkboxBlock.checked = false;
            checkboxBlock.removeAttribute('checked');
          }
...
this.updateVisibility = function() {
      ...
      var gateBlocked = openMapsWazeNotListedLayerUseBlocked(map, self.mapId);
...
// 3. Eye Icon (manual visibility) + Waze not_listed gate (padlock)
      UI.visibility.classList.remove('fa-eye', 'fa-eye-slash', 'fa-lock');
      if (gateBlocked) {
        UI.visibility.classList.add('fa-lock');
        UI.visibility.style.color = '#5f6368';
        UI.visibility.style.cursor = 'not-allowed';
        UI.visibility.style.pointerEvents = 'none';
        Tooltips.add(UI.visibility, I18n.t('openmaps.waze_not_listed_eye_locked'), true);
      } else {
        UI.visibility.style.pointerEvents = '';
      ...
      var showAccredGatePill = openMapsIsCatalogWazeNotListedMap(map) && Settings.get().developerMode &&
        !openMapsWazeNotListedSessionHasAgreed(self.mapId) &&
        !openMapsWazeNotListedSessionHasAgreed(map.id);
      if (UI.notAccreditedPill) {
        UI.notAccreditedPill.style.display = showAccredGatePill ? 'inline-flex' : 'none';
      }
      if (UI.accredPillRow) {
        UI.accredPillRow.style.display = showAccredGatePill ? 'flex' : 'none';
      }

      if (UI.mapLayersActionsBtn) {
        UI.mapLayersActionsBtn.disabled = !!gateBlocked;
        UI.mapLayersActionsBtn.style.opacity = gateBlocked ? '0.45' : '';
        UI.mapLayersActionsBtn.style.pointerEvents = gateBlocked ? 'none' : '';
      }
...
// 4. Bounding Box (Blocked State & UI Toggles)
      ...
      if (self.layerUI) {
        self.layerUI.forEach(lUI => {
          ...
          if (lUI.eyeBtn) {
            lUI.eyeBtn.style.opacity = gateBlocked ? '0.45' : '';
            lUI.eyeBtn.style.pointerEvents = gateBlocked ? 'none' : '';
          }
          lUI.title.style.pointerEvents = gateBlocked ? 'none' : '';
        });
      }
    };
...
/* Map card “Not accredited” chip: WME/wz buttons often impose a tall min-height — keep it compact */
#sidepanel-openMaps .openmaps-not-accredited-pill {
  margin: 0 !important;
...
#sidepanel-openMaps .openmaps-map-card-accred-pill-row {
  line-height: 1;
}
